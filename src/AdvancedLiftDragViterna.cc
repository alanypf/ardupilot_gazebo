...
#include "AdvancedLiftDragViterna.hh"
...
class gz::sim::systems::AdvancedLiftDragViternaPrivate
{
...
//////////////////////////////////////////////////
void AdvancedLiftDragViternaPrivate::Load(const EntityComponentManager &_ecm,
                           const sdf::ElementPtr &_sdf)
{
  ...
  */
  ...
  if (_sdf->HasElement("link_name"))
  {
    ...
    if (entities.empty())
    {
      gzerr << "Link with name[" << linkName << "] not found. "
             << "The AdvancedLiftDragViterna will not generate forces.\n";
      this->validConfig = false;
      return;
    }
    else if (entities.size() > 1)
  ...
  {
    gzerr << "AdvancedLiftDragViterna system requires the 'link_name' parameter.\n";
    this->validConfig = false;
    return;
  }
...
//////////////////////////////////////////////////
AdvancedLiftDragViterna::AdvancedLiftDragViterna()
    : System(), dataPtr(std::make_unique<AdvancedLiftDragViternaPrivate>())
{
}

//////////////////////////////////////////////////
void AdvancedLiftDragViternaPrivate::Update(EntityComponentManager &_ecm)
{
  GZ_PROFILE("AdvancedLiftDragViternaPrivate::Update");
  ...
  {
    CL = this->CL0 + this->CLa * this->alpha;
  }
  ...
  if (alpha > this->alphaStall)
  {
    CD = this->CD0 + (CL*CL) / (GZ_PI * this->AR * this->eff) +
  ...
  if (++debug_counter % 100 == 0) {
    gzmsg << "Viterna Debug -> alpha: " << this->alpha * 180.0 / GZ_PI 
          << " deg, beta: " << this->beta * 180.0 / GZ_PI 
          << " deg, CL: " << CL << ", CD: " << CD 
          << " | lift: " << lift << ", drag: " << drag 
...
//////////////////////////////////////////////////
void AdvancedLiftDragViterna::Configure(const Entity &_entity,
...
//////////////////////////////////////////////////
void AdvancedLiftDragViterna::PreUpdate(const UpdateInfo &_info,
  EntityComponentManager &_ecm)
{
  GZ_PROFILE("AdvancedLiftDragViterna::PreUpdate");
...
GZ_ADD_PLUGIN(AdvancedLiftDragViterna,
                    System,
                    AdvancedLiftDragViterna::ISystemConfigure,
                    AdvancedLiftDragViterna::ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(AdvancedLiftDragViterna,
                    "gz::sim::systems::AdvancedLiftDragViterna")
